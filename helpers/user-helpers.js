...
var objectId = require('mongodb').ObjectId;
 var instance = new Razorpay({
                key_id:'rzp_test_SN3dIh7DcfGhpS',
                key_secret:'U4dNdrHob1JFbNmZE0lD6cL0'
            })
module.exports = {
    doSignup:(userData)=>{
    ...
    addToCart:(proId,userId)=>{
        let proObj = {
         ...
         return new Promise(async(resolve, reject) => {
            ...
            if(userCart){
                let proExist = userCart.products.findIndex(product=> product.item && product.item.equals(new objectId(proId)))
    ...
    removeProduct:(details) =>{
        return new Promise((resolve, reject) => {
            db.get().collection(collection.CART_COLLECTION)
                .updateOne({_id: new objectId(details.cart)},
                            {$pull:{products:{item:new objectId(details.product)}}}).then((response)=>{
                                 resolve({removeProduct:true})
                            })
        })
    },
    ...
    verifyPayment:(details)=>{
        return new Promise((resolve, reject) => {
            ...
            let hmac = crypto.createHmac('sha256','U4dNdrHob1JFbNmZE0lD6cL0')
